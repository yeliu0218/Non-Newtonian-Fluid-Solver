...
DD_PLIC_Advection const*
DD_PLIC_Advection::PROTOTYPE = new DD_PLIC_Advection() ;

//---------------------------------------------------------------------------
DD_PLIC_Advection:: DD_PLIC_Advection( void )
//---------------------------------------------------------------------------
   : FE_OneStepIteration( "DD_PLIC_Advection" )
   , CC( 0 )
   , UU( 0 )
   , cFE( 0 )
   , sFE( 0 )
...
//---------------------------------------------------------------------------
DD_PLIC_Advection*
DD_PLIC_Advection:: create_replica( PEL_Object* a_owner,
		           PDE_DomainAndFields const* dom,
		           FE_SetOfParameters const* prms,
		           PEL_ModuleExplorer* exp ) const
//---------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: create_replica" ) ;
   PEL_CHECK( create_replica_PRE( a_owner, dom, prms, exp ) ) ;

   DD_PLIC_Advection* result = new DD_PLIC_Advection( a_owner, dom, prms, exp ) ;
...
//---------------------------------------------------------------------------
DD_PLIC_Advection:: DD_PLIC_Advection( PEL_Object* a_owner,
		     PDE_DomainAndFields const* dom,
		     FE_SetOfParameters const* prms,
   ...
   , Yvector( doubleVector( 0 ) )
{
   ...
   PEL_LABEL( "DD_PLIC_Advection:: DD_PLIC_Advection" ) ;
...
//-----------------------------------------------------------------------------
void
DD_PLIC_Advection:: do_before_inner_iterations_stage( FE_TimeIterator const* t_it )
//-----------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: do_before_inner_iterations_stage" ) ;
   PEL_CHECK_PRE( do_before_inner_iterations_stage_PRE( t_it ) ) ;

   start_total_timer( "DD_PLIC_Advection:: do_before_inner_iterations_stage" ) ;
   stop_total_timer() ;

   PEL_CHECK_POST( do_before_inner_iterations_stage_POST( t_it ) ) ;
}
//---------------------------------------------------------------------------
void DD_PLIC_Advection:: do_one_inner_iteration( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: do_one_inner_iteration" ) ;
   PEL_CHECK_PRE( do_one_inner_iteration_PRE( t_it ) ) ;

   start_total_timer( "DD_PLIC_Advection:: do_one_inner_iteration" ) ;
   // --------------
    
   PEL_MemoryTracer::object()->start_event(
      "DD_PLIC_Advection::do_one_inner_iteration \""+CC->name()+"\"" ) ;
...
//------------------------------------------------------------------------
void
DD_PLIC_Advection:: print_mesh( FE_TimeIterator const* t_it )
//------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: print_mesh" ) ;
...
//------------------------------------------------------------------------
void
DD_PLIC_Advection:: loop_on_cellsXX( FE_TimeIterator const* t_it )
//------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: loop_on_cellsXX" ) ;
   ...
   {
      n_C = cFE->global_node( CC, 0 ) ;
      if (n_C%NY==0 && CC->DOF_value(0,n_C+1)>Cmax)
         CC->set_DOF_value(0, n_C, CC->DOF_value(0,n_C+1));
      if (n_C%NY==0 && n_C/NY<Xlimit)
         CC->set_DOF_value(0, n_C, 1.0);
   }
}

//------------------------------------------------------------------------
void
DD_PLIC_Advection:: loop_on_cellsZZ( FE_TimeIterator const* t_it )
//------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: loop_on_cellsZZ" ) ;
   double const dt = 0.5*t_it->time_step() ;
   double s1,s2,s3,s4,mm1,mm2,V1,V2,mx,mz,alfa;
   size_t n_C, n_U, inv;
   for ( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
...
//-------------------------------------------------------------------------
void
DD_PLIC_Advection::do_inner_iterations_stage( FE_TimeIterator const* t_it )
//-------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: do_inner_iterations_stage" ) ;
...
//-------------------------------------------------------------------------
bool
DD_PLIC_Advection::AD_inner_iterations_are_completed( FE_TimeIterator * t_glo, FE_TimeIterator const* t_it, double epsilon ) const
//-------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: AD_inner_iterations_are_completed( t_global, t_it )" ) ;
...
//----------------------------------------------------------------------
void
DD_PLIC_Advection:: reset_discrete_problem( FE_TimeIterator const* t_it )
//----------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: reset_discrete_problem" ) ;
}
...
//-------------------------------------------------------------------------
void
DD_PLIC_Advection::update_fields( void )
//-------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: update_fields" ) ;
}

//-------------------------------------------------------------------------
double
DD_PLIC_Advection::smallest_vertices( void )
//-------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: smallest_vertices" ) ;
...
//-------------------------------------------------------------------------
double
DD_PLIC_Advection::time_step_by_Courant( double dt_new, FE_TimeIterator const* t_it )
//-------------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection:: time_step_by_Courant" ) ;
...
double
DD_PLIC_Advection::vol2( double mx, double mz, double alfa, double b)
//----------------------------------------------------------------------
{
   PEL_LABEL( "DD_PLIC_Advection::vol2" ) ;
   double alfaa,mb;
   mb=mx*b;
   if (alfa<=0.0)
      return 0.0;
