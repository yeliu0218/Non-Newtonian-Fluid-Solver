...
//------------------------------------------------------------------------
{
    ...
    for( sFE->start() ; sFE->is_valid() ; sFE->go_next() )
    {
        ...
        if( b0.size()==0 && b1.size()!=0 ) {
            bFE->go_i_th( b1( 0 ) ) ;
            GE_Vector const* bnormal = bFE->outward_normal() ;
            GE_Vector const* snormal = sFE->normal() ;
            double dot = bnormal->dot_product(snormal);
            if (dot!=0.0 && CC->DOF_value(0,n_C0)>Cmax) {
        ...
        if( b0.size()!=0 && b1.size()==0 ) {
            ...
            GE_Vector const* snormal = sFE->normal() ;
            double dot = bnormal->dot_product(snormal);
            if (dot!=0.0 && CC->DOF_value(0,n_C1)>Cmax) {
                //PEL::out() << indent() << " Error in computing the measure " << std::endl;
