...
MY_PLIC3_Advection const*
MY_PLIC3_Advection::PROTOTYPE = new MY_PLIC3_Advection() ;

//---------------------------------------------------------------------------
MY_PLIC3_Advection:: MY_PLIC3_Advection( void )
//---------------------------------------------------------------------------
    : FE_OneStepIteration( "MY_PLIC3_Advection" )
    , CC( 0 )
    , UU( 0 )
    , BCs( 0 )
    , cFE( 0 )
...
//---------------------------------------------------------------------------
MY_PLIC3_Advection*
MY_PLIC3_Advection:: create_replica( PEL_Object* a_owner,
                                    PDE_DomainAndFields const* dom,
                                    FE_SetOfParameters const* prms,
                                    PEL_ModuleExplorer* exp ) const
//---------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: create_replica" ) ;
    PEL_CHECK( create_replica_PRE( a_owner, dom, prms, exp ) ) ;

    MY_PLIC3_Advection* result = new MY_PLIC3_Advection( a_owner, dom, prms, exp ) ;
...
//---------------------------------------------------------------------------
MY_PLIC3_Advection:: MY_PLIC3_Advection( PEL_Object* a_owner,
                                       PDE_DomainAndFields const* dom,
                                       FE_SetOfParameters const* prms,
      ...
      //, Xcoord( doubleVector( 0 ) )
{
    ...
    PEL_LABEL( "MY_PLIC3_Advection:: MY_PLIC3_Advection" ) ;
...
void MY_PLIC3_Advection:: setup_connectivity() {  
...
void MY_PLIC3_Advection:: compute_gradient_C(size_t ic) {
    size_t n_C;
    ...
    for( cFE->start() ; cFE->is_valid() ; cFE->go_next() ) {
        ...
        if (n_L==-1) { //cell on the left bound
            if(n_U==-1) { //cell on the upper left corner, n_UL, n_U, n_L, n_UR, n_DL == -1
                GRADx(n_C) = (CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_C,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_C,ic))+(CC->DOF_value(0,n_DR,ic)-CC->DOF_value(0,n_D,ic));
                GRADz(n_C) = (CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_D,ic))+2.0*(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_D,ic))+(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_DR,ic));            
            }
            else if(n_D==-1) { //cell on the lower left corner, n_DL, n_D, n_L, n_DR, n_UL == -1
                GRADx(n_C) = (CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_U,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_C,ic))+(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_C,ic));
                GRADz(n_C) = (CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_C,ic))+2.0*(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_C,ic))+(CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_R,ic));
            }
            else { //cell inside the left bound, n_L, n_UL, n_DL == -1
                GRADx(n_C) = (CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_U,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_C,ic))+(CC->DOF_value(0,n_DR,ic)-CC->DOF_value(0,n_D,ic));
                GRADz(n_C) = (CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_D,ic))+2.0*(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_D,ic))+(CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_DR,ic));
                GRADx(n_C)*=2.0;
            }
        }
        else if(n_R==-1) { //cell on the right bound
            if(n_U==-1) { //cell on the upper right corner, n_UR, n_U, n_R, n_UL, n_DR == -1
                GRADx(n_C) = (CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_L,ic))+2.0*(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_D,ic)-CC->DOF_value(0,n_DL,ic));
                GRADz(n_C) = (CC->DOF_value(0,n_L,ic)-CC->DOF_value(0,n_DL,ic))+2.0*(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_D,ic))+(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_D,ic));
            }
            else if(n_D==-1) { //cell on the lower right corner, n_DR, n_D, n_R, n_UR, n_DL == -1
                GRADx(n_C) = (CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_UL,ic))+2.0*(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_L,ic));
                GRADz(n_C) = (CC->DOF_value(0,n_UL,ic)-CC->DOF_value(0,n_L,ic))+2.0*(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_C,ic))+(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_C,ic));
            }
            else { //cell inside the right bound, n_R, n_UR, n_DR == -1
                GRADx(n_C) = (CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_UL,ic))+2.0*(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_D,ic)-CC->DOF_value(0,n_DL,ic));
                GRADz(n_C) = (CC->DOF_value(0,n_UL,ic)-CC->DOF_value(0,n_DL,ic))+2.0*(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_D,ic))+(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_D,ic));
                GRADx(n_C)*=2.0;
            }
        }
        else if(n_U==-1) { //cell inside the upper bound, n_U, n_UL, n_UR == -1
            GRADx(n_C) = (CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_DR,ic)-CC->DOF_value(0,n_DL,ic));
            GRADz(n_C) = (CC->DOF_value(0,n_L,ic)-CC->DOF_value(0,n_DL,ic))+2.0*(CC->DOF_value(0,n_C,ic)-CC->DOF_value(0,n_D,ic))+(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_DR,ic));
            GRADz(n_C)*=2.0;
        }
        else if(n_D==-1) { //cell inside the lower bound, n_D, n_DL, n_DR == -1
            GRADx(n_C) = (CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_UL,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic));
            GRADz(n_C) = (CC->DOF_value(0,n_UL,ic)-CC->DOF_value(0,n_L,ic))+2.0*(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_C,ic))+(CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_R,ic));
            GRADz(n_C)*=2.0;
        }
        else if(n_UL==-1){
            GRADx(n_C) = (CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_DR,ic)-CC->DOF_value(0,n_DL,ic));
            ...
            GRADx(n_C) = (CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_UL,ic))+2.0*(CC->DOF_value(0,n_R,ic)-CC->DOF_value(0,n_L,ic))+(CC->DOF_value(0,n_DR,ic)-CC->DOF_value(0,n_DL,ic));
            GRADz(n_C) = (CC->DOF_value(0,n_UL,ic)-CC->DOF_value(0,n_DL,ic))+2.0*(CC->DOF_value(0,n_U,ic)-CC->DOF_value(0,n_D,ic))+(CC->DOF_value(0,n_UR,ic)-CC->DOF_value(0,n_DR,ic));
        }
    }
...
void
MY_PLIC3_Advection:: do_before_inner_iterations_stage( FE_TimeIterator const* t_it )
//-----------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: do_before_inner_iterations_stage" ) ;
    PEL_CHECK_PRE( do_before_inner_iterations_stage_PRE( t_it ) ) ;

    start_total_timer( "MY_PLIC3_Advection:: do_before_inner_iterations_stage" ) ;
    TT->set( t_it->time() ) ;
    stop_total_timer() ;
...
//---------------------------------------------------------------------------
void MY_PLIC3_Advection:: do_one_inner_iteration( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: do_one_inner_iteration" ) ;
    PEL_CHECK_PRE( do_one_inner_iteration_PRE( t_it ) ) ;

    start_total_timer( "MY_PLIC3_Advection:: do_one_inner_iteration" ) ;
    // --------------

    PEL_MemoryTracer::object()->start_event(
        "MY_PLIC3_Advection::do_one_inner_iteration \""+CC->name()+"\"" ) ;
    ...
    loop_on_cellsXX( t_it,0 ) ;
    loop_on_cellsZZ( t_it,0 ) ;
    loop_on_cellsZZ( t_it,0 ) ;
    loop_on_cellsXX( t_it,0 ) ;
    loop_on_cellsXX( t_it,1 ) ;
    loop_on_cellsZZ( t_it,1 ) ;
    loop_on_cellsZZ( t_it,1 ) ;
    loop_on_cellsXX( t_it,1 ) ;
    regularize_C(t_it);
...
//------------------------------------------------------------------------
void
MY_PLIC3_Advection:: print_mesh( FE_TimeIterator const* t_it )
//------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: print_mesh" ) ;
    for ( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
    {
...
void 
//----------------------------------------------------------------
MY_PLIC3_Advection::regularize_C(FE_TimeIterator const* t_it)
{
	double a,b;
	size_t n_C;
	for( cFE->start() ; cFE->is_valid() ; cFE->go_next() ) {
        n_C = cFE->global_node(CC, 0);
        a=CC->DOF_value(0,n_C,0);
        b=CC->DOF_value(0,n_C,1);
        if (a+b>1.0){
			CC->set_DOF_value(0,n_C,a/(a+b),0);
			CC->set_DOF_value(0,n_C,1.0-CC->DOF_value(0,n_C,0),1);
	    }
    }
}

void
MY_PLIC3_Advection:: loop_on_cellsXX( FE_TimeIterator const* t_it, size_t ic )
//------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: loop_on_cellsXX" ) ;
    double const dt = 0.5*t_it->time_step() ;
    double s0,d0,d1,s1,s2,mm1,mm2,V1,V2,mx,mz,alfa;
    size_t n_C0,n_C1,n_C,inv;
    doubleVector value(0);
    compute_gradient_C(ic);
    //size_t N = cFE->nb_meshes();
    C1.set(0.0);
    C2.set(0.0);
    C3.set(0.0);
    /*for( cFE->start() ; cFE->is_valid() ; cFE->go_next() ) {
        n_C = cFE->global_node(CC, 0);
        CC->set_DOF_value(1,n_C,0.0,ic);
        CC->set_DOF_value(2,n_C,0.0,ic);
        CC->set_DOF_value(3,n_C,0.0,ic);
    }*/
    ...
    for ( sFE->start() ; sFE->is_valid() ; sFE->go_next() ) {
        ...
        if (PEL::abs(normal->component(0))>1.0-1.E-6) {
            ...
            if(normal->component(0)>0.0) {
                //CC->set_DOF_value(3, n_C0, s0, ic);
                //CC->set_DOF_value(1, n_C1, s0, ic);
                C3(n_C0) = s0;
                C1(n_C1) = s0;
            }
            else {
                //CC->set_DOF_value(1, n_C0, s0, ic);
                //CC->set_DOF_value(3, n_C1, s0, ic);
                C1(n_C0) = s0;
                C3(n_C1) = s0;
            }
        }
    ...
    for( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
    {
        ...
        else if ( CC->DOF_value(0,n_C,ic)==1.0 )
        {
            /*s1=CC->DOF_value(1,n_C, ic);
            s2=CC->DOF_value(3,n_C, ic);
            CC->set_DOF_value(1, n_C, (-s1 > 0.0) ? -s1 : 0.0, ic );
            CC->set_DOF_value(2, n_C, 1.0 - (s1 > 0.0 ? s1 : 0.0) + ( s2 < 0.0 ? s2 : 0.0 ), ic);
            CC->set_DOF_value(3, n_C, s2 > 0.0 ? s2 : 0.0, ic);*/
            s1 = C1(n_C);
            s2 = C3(n_C);
            C1(n_C) = (-s1 > 0.0) ? -s1 : 0.0;
            C2(n_C) = 1.0 - (s1 > 0.0 ? s1 : 0.0) + ( s2 < 0.0 ? s2 : 0.0 );
            C3(n_C) = s2 > 0.0 ? s2 : 0.0;
        }
        else {
            //s1=CC->DOF_value(1,n_C,ic);
            //s2=CC->DOF_value(3,n_C,ic);
            ...
            V1=0.5*mm1;
            V2=1.0-V1;
            if (CC->DOF_value(0,n_C,ic)<=V1)
                alfa=sqrt(2.0*CC->DOF_value(0,n_C,ic)*mm1);
            else if (CC->DOF_value(0,n_C,ic)<=V2)
                alfa=CC->DOF_value(0,n_C,ic)+0.5*mm1;
            else
                alfa=mm1+1.0-sqrt(2.0*(1.0-CC->DOF_value(0,n_C,ic))*mm1);
            ...
            mm1=(-s1 > 0.0) ? -s1 : 0.0;
            mm2=alfa+mx*mm1;
            //CC->set_DOF_value(1, n_C, vol2(mx,mz,mm2,mm1),ic);
            C1(n_C) = vol2(mx,mz,mm2,mm1);

            mm1=1.0-(s1>0.0?s1:0.0)+(s2<0.0?s2:0.0);
            mm2=alfa-mx*(s1>0.0?s1:0.0);
            //CC->set_DOF_value(2, n_C, vol2(mx,mz,mm2,mm1),ic);
            C2(n_C) = vol2(mx,mz,mm2,mm1);

            mm1=s2>0.0?s2:0.0;
            mm2=alfa-mx;
            //CC->set_DOF_value(3, n_C, vol2(mx,mz,mm2,mm1),ic);
            C3(n_C) = vol2(mx,mz,mm2,mm1);

            // it is very likely that this is the problem!!
            if (inv==1)
            {
                //mm1=CC->DOF_value(1,n_C,ic);
                //CC->set_DOF_value(1, n_C, CC->DOF_value(3,n_C,ic),ic);
                //CC->set_DOF_value(3, n_C, mm1,ic);
                mm1 = C1(n_C);
                C1(n_C) = C3(n_C);
                C3(n_C) = mm1;
            }
    ...
    for( sFE->start() ; sFE->is_valid() ; sFE->go_next() )
    {
        ...
        if (PEL::abs(normal->component(0))>1.0-1.E-6) {
            ...
            if (normal->component(0)>0.0){				
				//CC->set_DOF_value(2, n_C0, CC->DOF_value(2, n_C0,ic)+CC->DOF_value(1, n_C1,ic),ic );
                //CC->set_DOF_value(2, n_C1, CC->DOF_value(2, n_C1,ic)+CC->DOF_value(3, n_C0,ic),ic );
                C2(n_C0) += C1(n_C1);
                C2(n_C1) += C3(n_C0);
            }
            else{
				//CC->set_DOF_value(2, n_C0, CC->DOF_value(2, n_C0,ic)+CC->DOF_value(3, n_C1,ic),ic );
                //CC->set_DOF_value(2, n_C1, CC->DOF_value(2, n_C1,ic)+CC->DOF_value(1, n_C0,ic),ic );
			    C2(n_C0) += C3(n_C1);
			    C2(n_C1) += C1(n_C0);
			}
        }
            ...
            if( BCs->has_BC( bFE->color(), CC ) ) {
                ...
                if( type == "Dirichlet" ) {
                    ...
                    if(value(ic) == 1.0) {
                        ...
                        //CC->set_DOF_value(2, n_C0, CC->DOF_value(2, n_C0,ic)+ (s0>0.0?s0:0.0),ic);
                        C2(n_C0) += (s0>0.0?s0:0.0);
                        //else {//left boundary
    ...
    for( cFE->start() ; cFE->is_valid() ; cFE->go_next() ) {
        n_C = cFE->global_node(CC, 0);
        //CC->set_DOF_value(0,n_C,CC->DOF_value(2,n_C,ic),ic);
        CC->set_DOF_value(0,n_C,C2(n_C),ic);
    }
    //regularize C
    for( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
    {
        n_C = cFE->global_node( CC, 0 ) ;
        mm1=0.0>CC->DOF_value(0,n_C,ic)?0.0:CC->DOF_value(0,n_C,ic);
        mm2=1.0<mm1?1.0:mm1;
        CC->set_DOF_value(0, n_C, mm2,ic);
    }
    ...
    for( sFE->start() ; sFE->is_valid() ; sFE->go_next() )
    {
        ...
        if( b0.size()==0 && b1.size()!=0 ) {
            if (CC->DOF_value(0,n_C0,ic)>Cmax) {
                ...
                if ( FE::geometry() == FE::axisymmetrical ){
					...
					increment+=(1.0-CC->DOF_value(0,n_C1,ic))*cFE1->calculation_point()->coordinate( 0 )*area;
                }
                else{
                    increment+=(1.0-CC->DOF_value(0,n_C1,ic))*area;
			    }
                CC->set_DOF_value(0,n_C1,1.0,ic);
            }
        }
        if( b0.size()!=0 && b1.size()==0 ) {
            if (CC->DOF_value(0,n_C1,ic)>Cmax) {
				PEL::out() << indent() << " Error in computing the measure " << std::endl;
		        area = cFE0->polyhedron()->measure();
                //or change 1.0 to CC->DOF_value(0,n_C1)
                if ( FE::geometry() == FE::axisymmetrical ){
					//cFE0->set_calculation_point( cFE0->polyhedron()->center() ) ;
					increment+=(1.0-CC->DOF_value(0,n_C0,ic))*cFE0->calculation_point()->coordinate( 0 )*area;
                }
                else{
                    increment+=(1.0-CC->DOF_value(0,n_C0,ic))*area;
			    }
                CC->set_DOF_value(0,n_C0,1.0,ic);
            }
        }
    ...
    if (increment>0.0)
    {
        ...
        update_interface(increment,ic);
    }
}

void
MY_PLIC3_Advection:: loop_on_cellsZZ( FE_TimeIterator const* t_it,size_t ic)
//------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: loop_on_cellsZZ" ) ;
    double const dt = 0.5*t_it->time_step() ;
    double s0,d0,d1,s1,s2,mm1,mm2,V1,V2,mx,mz,alfa;
    size_t n_C0,n_C1,n_C,inv;
    doubleVector value(0);
    ...
    for( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
    {
        ...
        else if ( CC->DOF_value(0,n_C,ic)==1.0 )
        {
            ...
            mm1=(-s1 > 0.0) ? -s1 : 0.0;
            mm2=alfa+mz*mm1;
            //CC->set_DOF_value(1, n_C, vol2(mz,mx,mm2,mm1),ic);
            C1(n_C) = vol2(mz,mx,mm2,mm1);

            mm1=1.0-(s1>0.0?s1:0.0)+(s2<0.0?s2:0.0);
            mm2=alfa-mz*(s1>0.0?s1:0.0);
            //CC->set_DOF_value(2, n_C, vol2(mz,mx,mm2,mm1),ic);
            C2(n_C) = vol2(mz,mx,mm2,mm1);

            mm1=s2>0.0?s2:0.0;
            mm2=alfa-mz;
            //CC->set_DOF_value(3, n_C, vol2(mz,mx,mm2,mm1),ic);
            C3(n_C) = vol2(mz,mx,mm2,mm1);
...
//-------------------------------------------------------------------------
void
MY_PLIC3_Advection::do_inner_iterations_stage( FE_TimeIterator const* t_it )
//-------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: do_inner_iterations_stage" ) ;
...
//-------------------------------------------------------------------------
bool
MY_PLIC3_Advection::AD_inner_iterations_are_completed( FE_TimeIterator * t_glo, FE_TimeIterator const* t_it, double epsilon ) const
//-------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: AD_inner_iterations_are_completed( t_global, t_it )" ) ;
...
//----------------------------------------------------------------------
void
MY_PLIC3_Advection:: reset_discrete_problem( FE_TimeIterator const* t_it )
//----------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: reset_discrete_problem" ) ;
}
...
//-------------------------------------------------------------------------
void
MY_PLIC3_Advection::update_fields( void )
//-------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: update_fields" ) ;
}

//-------------------------------------------------------------------------
double
MY_PLIC3_Advection::smallest_vertices( void )
//-------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: smallest_vertices" ) ;
...
//-------------------------------------------------------------------------
double
MY_PLIC3_Advection::time_step_by_Courant( double dt_new, FE_TimeIterator const* t_it )
//-------------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection:: time_step_by_Courant" ) ;
...
double
MY_PLIC3_Advection::vol2( double mx, double mz, double alfa, double b)
//----------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection::vol2" ) ;
    double alfaa,mb;
    mb=mx*b;
    if (alfa<=0.0)
        return 0.0;
...
void MY_PLIC3_Advection::update_interface(double diff,size_t ic)
//----------------------------------------------------------------------
{
    PEL_LABEL( "MY_PLIC3_Advection::update_interface" ) ;
    ...
    for( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
    {
        ...
        if (C<Cmax && C>0.0)  //one choice of interface
            CC->set_DOF_value(0, n_C, CC->DOF_value(0,n_C,ic)*ratio,ic);
    }
}
