...
ML_NavierStokes const* ML_NavierStokes::PROTOTYPE = new ML_NavierStokes() ;

//---------------------------------------------------------------------------
ML_NavierStokes:: ML_NavierStokes( void )
//--------------------------------------------------------------------------
        : FE_OneStepIteration( "ML_NavierStokes" )
        , COM(0)
        , COORDS(0)
{
}
...
//---------------------------------------------------------------------------
ML_NavierStokes*
                ML_NavierStokes:: create_replica( PEL_Object* a_owner,
                PDE_DomainAndFields const* dom,
                FE_SetOfParameters const* prms,
                PEL_ModuleExplorer* exp ) const
//---------------------------------------------------------------------------
{
        PEL_LABEL( "ML_NavierStokes:: create_replica" ) ;
        PEL_CHECK( create_replica_PRE( a_owner, dom, prms, exp ) ) ;

        ML_NavierStokes* result =
                        new ML_NavierStokes( a_owner, dom, prms, exp ) ;
...
//---------------------------------------------------------------------------
ML_NavierStokes:: ML_NavierStokes( PEL_Object* a_owner,
                                   PDE_DomainAndFields const* dom,
                                   FE_SetOfParameters const* prms,
...
//---------------------------------------------------------------------------
        ...
        , FUNC_E(1.0)
{
    PEL_LABEL( "ML_NavierStokes:: ML_NavierStokes" ) ;
...
//---------------------------------------------------------------------------
void
ML_NavierStokes:: do_one_inner_iteration( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   PEL_LABEL( "ML_NavierStokes:: do_one_inner_iteration" ) ;
   PEL_CHECK_PRE( do_one_inner_iteration_PRE( t_it ) ) ;

   //start_total_timer( "ML_NavierStokes:: do_one_inner_iteration" ) ;
...
// Loop for eventual flowrate steps
    ...
    UT_Viscoplastic::compute_strain_rate_tensor( cFE, UU, GAMMADOT); // Compute GAMMADOT(u)
    if(ViscoplasticType == "Bingham")
    	UT_Viscoplastic::update_strain_rate_tensor_d( cFE, STRESS,
             GAMMADOT, GAMMA, ViscoplasticAug, KAPPA, BN, FUNC_E, t_it );
    // Electrical field: FUNC_E =f(|E|) used to set B_new = B f(|E|)

    if(ViscoplasticType == "HerschelBulkley")
    {
    	UT_Viscoplastic::update_strain_rate_tensor_d_HB( cFE, STRESS,
    ...
    UT_Viscoplastic::update_Lagrange_multiplier( cFE, STRESS,
...
//---------------------------------------------------------------------------
void
ML_NavierStokes:: update_pressuredrop(  )
//---------------------------------------------------------------------------
{
 ...
 */
void
ML_NavierStokes:: setup_Outer( FE_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{

	PEL_LABEL( "ML_NavierStokes:: setup_Outer" ) ;
 ...
 */
void
ML_NavierStokes:: setup_Inner( FE_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: setup_Inner" ) ;
 ...
 */
void
ML_NavierStokes:: loop_on_cells_Outer( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
        PEL_LABEL( "ML_NavierStokes:: loop_on_cells_Outer" ) ;
 ...
 */
void
ML_NavierStokes:: loop_on_cells_Inner( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: loop_on_cells_Inner" ) ;
	size_t nbc = UU->nb_components() ;
	size_t nb_dims = cFE->nb_space_dimensions() ;
	PEL_ASSERT( nbc == nb_dims ) ;
}
 ...
 */
void
ML_NavierStokes:: loop_on_bounds_Outer( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
    PEL_LABEL( "ML_NavierStokes:: loop_on_bounds" ) ;
 ...
 */
void
ML_NavierStokes:: loop_on_bounds_Inner( FE_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: loop_on_bounds_Inner" ) ;
}

//---------------------------------------------------------------------------
void
ML_NavierStokes:: do_before_time_stepping( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: do_before_time_stepping" ) ;

	start_total_timer( "ML_NavierStokes:: do_before_time_stepping" ) ;
 ...
 */
void
ML_NavierStokes:: do_before_inner_iterations_stage( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   PEL_LABEL( "ML_NavierStokes:: do_before_inner_iterations_stage" ) ;
   PEL_CHECK_PRE( do_before_inner_iterations_stage_PRE( t_it ) ) ;

   start_total_timer( "ML_NavierStokes:: do_before_inner_iterations_stage" ) ;
...
//----------------------------------------------------------------------------
void
ML_NavierStokes::do_inner_iterations_stage( FE_TimeIterator const* t_it )
{
	PEL_LABEL( "ML_NavierStokes:: do_inner_iterations_stage" ) ;
    start_total_timer( "ML_NavierStokes:: do_inner_iterations_stage" ) ;
 ...
 */
bool
ML_NavierStokes:: inner_iterations_are_completed( FE_TimeIterator const* t_it ) const
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: inner_iterations_are_completed" ) ;
	if ( Viscoplastic )
	{
        ...
        double error_strainrate=UT_Viscoplastic::compute_norm_Dminusd( cFE, GAMMA, GAMMADOT );
 ...
 */
double
ML_NavierStokes::compute_L2_norm( PDE_DiscreteField const * FIELD,
								  size_t level1, size_t level2) const
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: compute_L2_square_norm" ) ;
 ...
 */
double
ML_NavierStokes::compute_flowrate( PDE_DiscreteField const * FIELD,
									size_t level1) const
//---------------------------------------------------------------------------
{
	PEL_LABEL( "ML_NavierStokes:: compute_L2_square_norm" ) ;
 ...
 */
double
ML_NavierStokes::param_euclidian_norm( FE_TimeIterator const* t_it , FE_Parameter const * param) const
//---------------------------------------------------------------------------
{
    PEL_LABEL( "ML_NavierStokes:: param_euclidian_norm" ) ;
 ...
 */
double
ML_NavierStokes::param_yield_stress_function( double const param_norm) const
//---------------------------------------------------------------------------
{
    PEL_LABEL( "ML_NavierStokes:: param_euclidian_norm" ) ;
