...
AS_NavierStokes const* AS_NavierStokes::PROTOTYPE = new AS_NavierStokes() ;

//---------------------------------------------------------------------------
AS_NavierStokes:: AS_NavierStokes( void )
//--------------------------------------------------------------------------
        : FE_OneStepIteration( "AS_NavierStokes" )
        , COM(0)
        , COORDS(0)
{
}
...
//---------------------------------------------------------------------------
AS_NavierStokes*
                AS_NavierStokes:: create_replica( PEL_Object* a_owner,
                PDE_DomainAndFields const* dom,
                FE_SetOfParameters const* prms,
                PEL_ModuleExplorer* exp ) const
//---------------------------------------------------------------------------
{
        PEL_LABEL( "AS_NavierStokes:: create_replica" ) ;
        PEL_CHECK( create_replica_PRE( a_owner, dom, prms, exp ) ) ;

        AS_NavierStokes* result =
                        new AS_NavierStokes( a_owner, dom, prms, exp ) ;
...
//---------------------------------------------------------------------------
AS_NavierStokes:: AS_NavierStokes( PEL_Object* a_owner,
                                   PDE_DomainAndFields const* dom,
                                   FE_SetOfParameters const* prms,
...
//---------------------------------------------------------------------------
        ...
        , FUNC_E(1.0)
{
    PEL_LABEL( "AS_NavierStokes:: AS_NavierStokes" ) ;
...
//---------------------------------------------------------------------------
void
AS_NavierStokes:: do_one_inner_iteration( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   PEL_LABEL( "AS_NavierStokes:: do_one_inner_iteration" ) ;
   PEL_CHECK_PRE( do_one_inner_iteration_PRE( t_it ) ) ;

   //start_total_timer( "AS_NavierStokes:: do_one_inner_iteration" ) ;
...
// Loop for eventual flowrate steps
    ...
    AS_Viscoplastic::compute_strain_rate_tensor( cFE, UU, GAMMADOT); // Compute GAMMADOT(u)
    if(ViscoplasticType == "Bingham")
    	AS_Viscoplastic::update_strain_rate_tensor_d( cFE, STRESS,
             GAMMADOT, GAMMA, ViscoplasticAug, KAPPA, BN, FUNC_E, t_it );
    // Electrical field: FUNC_E =f(|E|) used to set B_new = B f(|E|)

    if(ViscoplasticType == "HerschelBulkley")
    {
    	AS_Viscoplastic::update_strain_rate_tensor_d_HB( cFE, STRESS,
    ...
    AS_Viscoplastic::update_Lagrange_multiplier( cFE, STRESS,
...
//---------------------------------------------------------------------------
void
AS_NavierStokes:: update_pressuredrop(  )
//---------------------------------------------------------------------------
{
 ...
 */
void
AS_NavierStokes:: setup_Outer( FE_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{

	PEL_LABEL( "AS_NavierStokes:: setup_Outer" ) ;
 ...
 */
void
AS_NavierStokes:: setup_Inner( FE_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: setup_Inner" ) ;
 ...
 */
void
AS_NavierStokes:: loop_on_cells_Outer( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
        PEL_LABEL( "AS_NavierStokes:: loop_on_cells_Outer" ) ;
        ...
        for ( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
        {
            ...
            for ( ; cFE->valid_IP() ; cFE->go_next_IP() )
            {
                ...
                //by Leo
                double kappa_tmp = KAPPA->cell_value_at_IP( t_it, cFE, 0 ) ; 
                //end
                FE::add_grad_row_D_col_S( ELEMENT_EQ, cFE, ViscoplasticAug+kappa_tmp ) ; // relaxed "Viscosity" form
             }
    ...
    for ( cFE->start() ; cFE->is_valid() ; cFE->go_next() )
    {
        ...
        for (  ; cFE->valid_IP() ; cFE->go_next_IP() )
        {
            FE_TensorFormAssembling::add_grad_row_col_A( ELEMENT_EQ, cFE, -1. ) ;
        }
 ...
 */
void
AS_NavierStokes:: loop_on_cells_Inner( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: loop_on_cells_Inner" ) ;
	size_t nbc = UU->nb_components() ;
	size_t nb_dims = cFE->nb_space_dimensions() ;
	PEL_ASSERT( nbc == nb_dims ) ;
}
 ...
 */
void
AS_NavierStokes:: loop_on_bounds_Outer( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
    PEL_LABEL( "AS_NavierStokes:: loop_on_bounds" ) ;
 ...
 */
void
AS_NavierStokes:: loop_on_bounds_Inner( FE_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: loop_on_bounds_Inner" ) ;
}

//---------------------------------------------------------------------------
void
AS_NavierStokes:: do_before_time_stepping( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: do_before_time_stepping" ) ;

	start_total_timer( "AS_NavierStokes:: do_before_time_stepping" ) ;
 ...
 */
void
AS_NavierStokes:: do_before_inner_iterations_stage( FE_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   PEL_LABEL( "AS_NavierStokes:: do_before_inner_iterations_stage" ) ;
   PEL_CHECK_PRE( do_before_inner_iterations_stage_PRE( t_it ) ) ;

   start_total_timer( "AS_NavierStokes:: do_before_inner_iterations_stage" ) ;
...
//----------------------------------------------------------------------------
void
AS_NavierStokes::do_inner_iterations_stage( FE_TimeIterator const* t_it )
{
	PEL_LABEL( "AS_NavierStokes:: do_inner_iterations_stage" ) ;
    start_total_timer( "AS_NavierStokes:: do_inner_iterations_stage" ) ;
 ...
 */
bool
AS_NavierStokes:: inner_iterations_are_completed( FE_TimeIterator const* t_it ) const
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: inner_iterations_are_completed" ) ;
	if ( Viscoplastic )
	{
        ...
        double error_strainrate=AS_Viscoplastic::compute_norm_Dminusd( cFE, GAMMA, GAMMADOT );
 ...
 */
double
AS_NavierStokes::compute_L2_norm( PDE_DiscreteField const * FIELD,
								  size_t level1, size_t level2) const
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: compute_L2_square_norm" ) ;
 ...
 */
double
AS_NavierStokes::compute_flowrate( PDE_DiscreteField const * FIELD,
									size_t level1) const
//---------------------------------------------------------------------------
{
	PEL_LABEL( "AS_NavierStokes:: compute_L2_square_norm" ) ;
 ...
 */
double
AS_NavierStokes::param_euclidian_norm( FE_TimeIterator const* t_it , FE_Parameter const * param) const
//---------------------------------------------------------------------------
{
    PEL_LABEL( "AS_NavierStokes:: param_euclidian_norm" ) ;
 ...
 */
double
AS_NavierStokes::param_yield_stress_function( double const param_norm) const
//---------------------------------------------------------------------------
{
    PEL_LABEL( "AS_NavierStokes:: param_euclidian_norm" ) ;
