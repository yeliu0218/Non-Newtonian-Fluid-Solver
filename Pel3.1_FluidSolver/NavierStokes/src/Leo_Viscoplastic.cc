...
/*----------------------------------------------------------------------*/
void Leo_Viscoplastic::compute_strain_rate_tensor( PDE_LocalFEcell* cFE,
        PDE_DiscreteField const* UU,
        PDE_DiscreteField* gammadot)
{
    PEL_LABEL( "Leo_Viscoplastic:: compute_strain_rate_tensor" ) ;

    size_t dimension = UU->nb_components(), DcompNumber=0 ;
    size_t node=0;

    if (dimension == 2 ) DcompNumber=3;
    ...
    else
    {
        ...
        cout << "    Leo_Viscoplastic:: compute_strain_rate_tensor !!!" << endl;
        PEL_Error::exit();
    }
 ...
 */
/*------------------------------------------------------------------------*/
pair<size_t,double> Leo_Viscoplastic::strain_rate_component_number(
    size_t const &dimension,
    size_t const& compIdx, size_t const &direcIdx )
{
    PEL_LABEL( "Leo_Viscoplastic:: strain_rate_component_number" ) ;
    ...
    if (dimension == 2)
    {
        ...
        case 2 :
            ...
            break;
        default :
			PEL::out() << "Error in Leo_Viscoplastic:: strain_rate_component_number " << std::endl;
        	PEL_Error:: exit() ;
        }
    }
    else if (dimension == 3)
    ...
    else
    {
        cout << "!!! Dimension should be either 2 or 3" << endl;
        cout << "    Dimension " << dimension << " is not allowed in " << endl;
        cout << "    Leo_Viscoplastic:: strain_rate_component_number !!!" << endl;
        PEL_Error::exit();
    }
...
//-------------------------------------------------------------
void Leo_Viscoplastic::update_Lagrange_multiplier( PDE_LocalFEcell* cFE,
        PDE_DiscreteField* LAMBDA,
        PDE_DiscreteField const* gammadot,
        PDE_DiscreteField const* gamma,
        double const& aug_param)
{
    PEL_LABEL( "Leo_Viscoplastic:: update_Lagrange_multiplier" ) ;
...
*/
/*-----------------------------------------------------------*/
void Leo_Viscoplastic::update_strain_rate_tensor_d( PDE_LocalFEcell* cFE,
...
{
    PEL_LABEL( "Leo_Viscoplastic:: update_strain_rate_tensor_d" ) ;
...
*/
/*-----------------------------------------------------------*/
void Leo_Viscoplastic::update_strain_rate_tensor_d_HB( PDE_LocalFEcell* cFE,
        PDE_DiscreteField const* LAMBDA,
        PDE_DiscreteField const* gammadot,
        PDE_DiscreteField* gamma,
        double const& aug_param,
...
*/
/*---------------------------------------------------------------------*/
double Leo_Viscoplastic::Euclidian_norm( doubleVector const& tensor )
{
    PEL_LABEL( "Leo_Viscoplastic:: Euclidian_norm" ) ;
	PEL_CHECK_PRE( tensor.size()==3 || tensor.size()==6 );
    double norm=0;

    if (tensor.size() == 3)
    {
        norm=0.5*(pow(tensor(0),2.)+pow(tensor(1),2.)+pow(tensor(0)+tensor(1),2.))+pow(tensor(2),2.);
        norm=sqrt(norm);
    }
    ...
    {
        cout << "!!! Number of components of tensor should be either 3 or 6"
        << endl;
        cout << "    Number of components " << tensor.size() << " is not allowed ";
        cout << " in" << endl;
        cout << "    Leo_Viscoplastic:: Euclidian_norm !!!" << endl;
        PEL_Error::exit();
    }
...
/*------------------------------------------------------------*/
double Leo_Viscoplastic::compute_norm_Dminusd( PDE_LocalFEcell* cFE,
        PDE_DiscreteField const* DUU,
        PDE_DiscreteField const* d)
{
    PEL_LABEL( "Leo_Viscoplastic:: compute_norm_Dminusd" ) ;
